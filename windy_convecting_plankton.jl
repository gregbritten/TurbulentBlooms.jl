...
Qu = - abs(args["momentum-flux"]) # Surface heat flux (W m⁻²)
...
initial_time_step = 30 # s
  ...
  output_interval = hour / 2
    output_prefix = @sprintf("convecting_plankton_Qh%04d_Qu%.2e_Nh%03d_Nz%03d", Qh, abs(Qu), Nh, Nz)
...
@info """ Parameters for demonstrating the critical turbulence hypothesis
    ...
    Momentum flux:                     $(@sprintf("%.2e", Qu)) m² s⁻²
...
u_bcs = UVelocityBoundaryConditions(grid, top = BoundaryCondition(Flux, Qu))
...
if !just_remake_animation # actually run the simulation
    ...
    model = IncompressibleModel(
        ...
        boundary_conditions = (b=buoyancy_bcs, u=u_bcs)
    )
...
try
    ...
    gif(anim, output_prefix * ".gif", fps = 8) # hide
